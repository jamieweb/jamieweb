...
<div class="body">
    ...
    <hr>
    <p><b>Saturday 19th January 2019</b></p>
...
&#x2517&#x2501&#x2501 <a href="#conclusion">Conclusion</a></pre>
    ...
    <h4 id="implementing-the-code-with-apache-and-php-mpm-event-proxy-fcgi">When running PHP with <code>mpm_event</code> and <code>proxy_fcgi</code>:</h4>
    <p>When you have PHP set up using <code>mpm_event</code> and <code>proxy_fcgi</code>, headers are put into the <code>always</code> list, so no further configuration should be required.</p>
    <h4 id="implementing-the-code-with-apache-and-php-mod-php-mpm-prefork">When running PHP with <code>mod_php</code> and <code>mpm_prefork</code>:</h4>
    <p>Headers set using PHP with <code>mod_php</code> and <code>mpm_prefork</code> go into the <code>onsuccess</code> list, and they can be overridden by Apache. For example, if you set a header in PHP implemented with <code>mod_php</code> using the <code>header()</code> function, but there is also a header with the same name in the Apache configuration set using <code>Header set</code>, the header set with Apache will be given priority, and a single header will be returned. On the other hand, if you have a header set using <code>header()</code>, and then another with the same name set in the Apache configuration using <code>Header always set</code>, both headers will be returned, resulting in a duplicate.</p>
    <p>It is important that your Content-Security-Policy is served on all pages, including your error pages and other unsuccessful requests, so the <code>always</code> list should be used. However, since <code>mod_php</code> will be setting a header in the <code>onsuccess</code> list, this will result in a duplicate header by default, which is not good.</p>

...
if(json_last_error() !== JSON_ERROR_NONE) {
    ...
    <p><i>One of the challenges with this setup is HSTS Preload. Since my domain (jamieweb.net) is HSTS Preloaded with includeSubDomains set, the crawler will only allow the connection to the local site over HTTPS. In order to resolve this, a self-signed TLS certificate is generated during the build and added to the nssdb, which is the trusted certificate authority list used by Chrome/Chromium on Linux. Apache is also configured to serve this certificate, meaning that the crawler is able to crawl the site without any HTTPS errors.</i></p>
    <p><i>If you wish to integrate this with your Report URI account, simply add your Report URI reporting address to the <code>csp.txt</code> file. You are able to specify multiple endpoints for the report-uri directive, so it is recommended to leave the localhost reporting endpoint on there so that you can still see the reports in your build log too.</i></p>

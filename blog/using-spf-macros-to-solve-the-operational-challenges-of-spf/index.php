...
&#x2517&#x2501&#x2501 <a href="#conclusion">Conclusion</a></pre>
    ...
    <ul class="large-spaced-list">
        ...
        <li><code>%{d}</code>: The authoritative domain of the current SPF policy --- usually the sender domain (<code>%{o}</code>), but may also represent other domains when evaluating <code>include:</code> statements.</li>
        <li><code>%{i}</code>: The source IP address of the message, e.g. <code>203.0.113.1</code>.</li>
        <li><code>%{p}</code>: The validated reverse-DNS domain of the source IP, e.g. if <code>example.com IN A</code> is <code>203.0.113.1</code> and <code>1.113.0.203.in-addr.arpa IN PTR</code> is <code>example.com</code>, the validated domain will be <code>example.com</code>.</li>
        ...
        <li><code>%{h}</code>: The domain used in the most recent SMTP <code>HELO</code> or <code>EHLO</code> command, e.g. <code>mail.example.com</code>.</li>
    </ul>
...
203.0.113.2._spf.example.com IN A 127.0.0.2</pre>
    ...
    <p>For example:</p>
    <pre>example.com IN TXT "v=spf1 exists:%{ir}.%{v}.arpa._spf.example.com -all"</pre>
    ...
    <p>Using SPF macros, it is possible to restrict third-party services who send on your behalf to a single or small number of addresses. This is great hardening and defence-in-depth, and also helps to keep a tight grip on the list of people authorised to send as your domain.</p>
    <p>The following example will allow a third-party service to send email as your domain, but <b>only</b> using <code>noreply@example.com</code>. Other SMTP <code>FROM</code> addresses will be treated as an SPF fail:</p>
    ...
    <p>Note that the automated account setup within some email services will fail to validate/accept your domain if you set it up this way, as they usually just do a basic Grep against your policy without actually evaluating macros. This can usually be solved by either adding the SPF macro after your domain has been verified, or by politely asking customer support to manually verify it for you.</p>
